		...
		elseif ( strcasecmp( $item->attr_title, 'disabled' ) == 0 ) :
			...
			*/
			...
			if ( ! empty( $icon ) ) :
				...
				$item_output .= '<a' . $attributes . '><span class="menu-text">';
			endif;
			$item_output .= $args->link_before . apply_filters( 'the_title', $item->title,
					$item->ID ) . $args->link_after;
			$item_output .= ( $args->has_children && 0 === $depth ) ? ' </span><span class="puur-caret"><i class="puur-dropdown-indicator"></i></span></a>' : '</span></a>';
